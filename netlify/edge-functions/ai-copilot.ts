...
export default async (request: Request, context: Context): Promise<Response> => {
  ...
  if (user.subscription === 'free') {
    return new Response(JSON.stringify({ error: 'AI Co-Pilot requires Pro subscription' }), { status: 403 });
  }
  ...
  return new Response(JSON.stringify({
    id: `chat-${Date.now()}`, role: 'assistant', content: response,
    timestamp: new Date().toISOString(), model: 'petropulse-copilot-v1'
